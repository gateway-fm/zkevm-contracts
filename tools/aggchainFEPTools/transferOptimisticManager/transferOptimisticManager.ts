...
async function main() {
    ...
    logger.info(`optimisticManager: ${optimisticManager.address} newOptimisticManager: ${newOptimisticManager.address}`);
    ...
    if(type === transactionTypes.TIMELOCK){
        ...
        outputJson.transferOptimisticModeManagerRole.decodedScheduleData =await decodeScheduleData(scheduleData, AggchainFEPFactory);
        ...
        // Decode the scheduleData for better readability
        outputJson.acceptOptimisticModeManagerRole.decodedScheduleData = await decodeScheduleData(scheduleData2, AggchainFEPFactory);
        ...
        const txTransferOptimiticModeManagerRole = AggchainFEPFactory.interface.encodeFunctionData("transferOptimisticModeManagerRole",
            [newOptimisticManager.address]
        );
        logger.info("Creating calldata to accept optimistic mode manager role from multisig...");
        const txAcceptOptimiticModeManagerRole = AggchainFEPFactory.interface.encodeFunctionData("acceptOptimisticModeManagerRole", []);
