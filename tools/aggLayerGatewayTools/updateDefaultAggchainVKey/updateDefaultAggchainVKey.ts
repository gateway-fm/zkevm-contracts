...
async function main() {
    ...
    if(type === transactionTypes.TIMELOCK){
        ...
        const txUpdateDefaultAggchainVKey = AggLayerGatewayFactory.interface.encodeFunctionData("updateDefaultAggchainVKey", [
            defaultAggchainSelector,
            newDefaultAggchainVKey
        ]);
        ...
        try {
            const tx = await aggLayerGateway.updateDefaultAggchainVKey(
                defaultAggchainSelector,
                newDefaultAggchainVKey
            );
            await tx.wait();
